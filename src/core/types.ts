...
export interface MenuItem {
  ...
  image: string;
  isWeightBased?: boolean; // Optional: true if this item is sold by weight
  weightPricePerKg?: number; // Optional: price per kilogram for weight-based items
  weightInGrams?: number; // Optional: weight in grams (used after modal confirmation)
}

export interface OrderItem extends MenuItem {
  quantity: number;
  weightInGrams?: number; // Optional: weight in grams for weight-based items
}
