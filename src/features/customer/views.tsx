...
export const MenuView = ({
  ...
  // Handle item click - open modal for weight-based items
      ...
      <WeightOrderModal
        ...
        onConfirm={(weightInGrams: number, price: number) => {
          if (selectedWeightItem && weightInGrams > 0 && price > 0) {
            ...
            // Directly update cart to bypass addToCart's weight-based check
            // which would otherwise re-open the modal
            setCart(prev => {
              ...
              <div key={item.id} className="flex items-center gap-3">
                ...
                <div className="flex items-center bg-gray-100 rounded-lg p-1">
                  <button onClick={() => removeFromCart(item.id)} className="p-1 hover:bg-gray-200 rounded-md transition-colors">
                    {item.quantity === 1 && !item.weightInGrams ? <Trash2 className="w-3 h-3 text-red-500" /> : <Minus className="w-3 h-3" />}
                  </button>
                  <span className="px-2 text-xs font-black">
                    {item.weightInGrams ? `${item.weightInGrams}g` : item.quantity}
                  </span>
                  {/* For weight-based items, don't show plus button since quantity is always 1 */}
                  {!item.weightInGrams && (
